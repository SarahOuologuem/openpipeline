...
thisConfig = processConfig(jsonSlurper.parseText('''{
  ...
  "info" : {
    "config" : "/home/runner/work/openpipeline/openpipeline/src/files/make_params/config.vsh.yaml",
    "platform" : "nextflow",
...
par <- list(
  "base_dir" = $( if [ ! -z ${VIASH_PAR_BASE_DIR+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_BASE_DIR" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "pattern" = $( if [ ! -z ${VIASH_PAR_PATTERN+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_PATTERN" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "n_dirname_drop" = $( if [ ! -z ${VIASH_PAR_N_DIRNAME_DROP+x} ]; then echo -n "as.integer('"; echo -n "$VIASH_PAR_N_DIRNAME_DROP" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "')"; else echo NULL; fi ),
  "n_basename_id" = $( if [ ! -z ${VIASH_PAR_N_BASENAME_ID+x} ]; then echo -n "as.integer('"; echo -n "$VIASH_PAR_N_BASENAME_ID" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "')"; else echo NULL; fi ),
  "id_name" = $( if [ ! -z ${VIASH_PAR_ID_NAME+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_ID_NAME" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "path_name" = $( if [ ! -z ${VIASH_PAR_PATH_NAME+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_PATH_NAME" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "group_name" = $( if [ ! -z ${VIASH_PAR_GROUP_NAME+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_GROUP_NAME" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
